...
Serial_Access_LL::Serial_Access_LL(QString interface, QString setting)
{
...
Serial_Access_LL::~Serial_Access_LL()
{
    Serial_disconnect();
}


int Serial_Access_LL::Serial_send(QString cmd)
{
...
QString Serial_Access_LL::Serial_get(QString cmd,int bufSize)
{
    QString qs_return;
...
int Serial_Access_LL::Serial_open(QString pr_interface)
{
    int i_return = 0;
...
int Serial_Access_LL::Serial_get_connected()
{
    return ser_connected;
}
int Serial_Access_LL::Serial_connect(QString pr_interface) {
    int i_return = 0;
...
int Serial_Access_LL::Serial_disconnect()
{
    if (fd_interface > 0)
        close(fd_interface);
...
int
Serial_Access_LL::set_interface_attribs (int fd, int speed, int parity)
{
    struct termios tty;
...
int
Serial_Access_LL::set_blocking (int fd, int should_block)
{
    struct termios tty;
    memset (&tty, 0, sizeof tty);
    if (tcgetattr (fd, &tty) != 0)
